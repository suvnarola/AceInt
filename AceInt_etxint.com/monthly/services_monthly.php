<?
 ...
 $date = date("Y-m-d", mktime(0,0,0,date("m"),date("d"),date("Y")));
  ...
  // get the actual taxinvoice ready.
   ...
   $buffer = energymonthly();
   $buffer2 = phonemonthly();
  ...
  $mail->add_attachment($buffer, $date.' Energy.txt', 'text/plain');
  ...
  // send the message.
   $mail->send('dave', 'dave@ebanctrade.com', 'E Banc Accounts - Head Office', 'accounts@ebanctrade.com', 'myServicesBanc - Monthly Uploads');


   $text = get_services_template('1', 'Les', 'Please find attacted account allocation for this week. Fund will be credited into your account overnight.');
...
function energymonthly() {

 global $linkid, $db;

 $disdate = date("Y-m-d", mktime(0,0,0,date("m"),21,date("Y")));
 $date3 = date("Y-m-d", mktime(0,0,0,date("m"),1-1,date("Y")));

 $SQLQuery = dbRead("select registered_accounts.* from registered_accounts, plans where (registered_accounts.Plan_ID = plans.FieldID) and ServiceID = 3 and Status_ID in (2,3) and Payments_Left > 0 and Date_Paid <= '".$date3."' order by Acc_No","ebanc_services");
 while($row = mysql_fetch_assoc($SQLQuery)) {
  ...
  $terms = $row['Terms'];
  $details = "Monthly Benefit to ".$row['Card_No'];
  ...
  if($row['Status_ID'] == 2) {
    ...
    $blah .= "$billcode,$card,$company,$payment,$terms, - $regacc - $accno\r\n";
  } else {
    $blah3 .= "$billcode,$card,$company,$payment,$terms, - $regacc - $accno - ".$row['Payments_Left']."\r\n";
  }
...
function phonemonthly() {
 ...
 while($row = mysql_fetch_assoc($SQLQuery)) {
  ...
  $payment = $row['Plan_Amount'];
  $details2 = "Monthly Benefit";
  $details = "Monthly Benefit to ".$row['Card_No'];

  $blah .= "$card,$companyname,$accholder,$payment,$details2\r\n";
...
function ratesmonthly() {
 ...
 $SQLQuery = dbRead("select registered_accounts.* from registered_accounts, plans, reg_acc_details where (registered_accounts.Plan_ID = plans.FieldID) and (registered_accounts.Acc_No = reg_acc_details.Acc_No) and ServiceID = 7 and Status_ID in (2,3) and Payments_Left > 0 and Date_Paid <= '".$date3."' order by registered_accounts.Acc_No","ebanc_services");
 while($row = mysql_fetch_assoc($SQLQuery)) {
  ...
  if($row['Status_ID'] == 2) {
    $date2 = servicedates($row['FieldID'],2);
    dbWrite("update registered_accounts set Status_ID = 3, Date_Renewal = '".date("Y-m-d", strtotime($date2))."' where FieldID = ".$row['FieldID']."","ebanc_services");
    $blah .= "$billcode,$card,$company,$payment,$terms, - $regacc - $accno - new\r\n";
  } else {
    $blah3 .= "$billcode,$card,$company,$payment,$terms, - $regacc - $accno - ".$row['Payments_Left']."\r\n";
  }
