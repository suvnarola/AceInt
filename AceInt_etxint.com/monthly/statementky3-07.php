...
function statement($query, $printStationery = false, $individual = false, $send = false, $senddate = false, $nomonths = false) {
 ...
 if($row['CID'] == 155) {
  ...
  while($row3 = mysql_fetch_array($query3)) {
   ...
   pdf_set_text_pos($pdf, 35, 570-$offset3);
   pdf_continue_text($pdf, $box1);

   pdf_set_text_pos($pdf, 100, 570-$offset3);
   pdf_continue_text($pdf, $box2);

   pdf_set_text_pos($pdf, get_right_pos($box33, $pdf, "286"), 570-$offset3);
   pdf_continue_text($pdf, $box33);

   pdf_set_text_pos($pdf, get_right_pos($box3, $pdf, "342"), 570-$offset3);
   pdf_continue_text($pdf, $box3);

   pdf_set_text_pos($pdf, get_right_pos($box4, $pdf, "402"), 570-$offset3);
   pdf_continue_text($pdf, $box4);

   pdf_set_text_pos($pdf, get_right_pos($box41, $pdf, "462"), 570-$offset3);
   pdf_continue_text($pdf, $box41);

   pdf_set_text_pos($pdf, get_right_pos($box5, $pdf, "512"), 570-$offset3);
   pdf_continue_text($pdf, $box5);
   ...
   //3 top lines in boxes
   pdf_setlinewidth($pdf, 1.5);
   pdf_moveto($pdf, 35, 555-$offset3);
   pdf_lineto($pdf, 560, 555-$offset3);
   pdf_stroke($pdf);
  ...
  pdf_set_text_pos($pdf, get_right_pos($row['sto'].":", $pdf, "290"), 570-$offset3);
  pdf_continue_text($pdf, $row['sto'].":");

  pdf_set_text_pos($pdf, get_right_pos(number_format($tbuy,2), $pdf, "342"), 570-$offset3);
  pdf_continue_text($pdf, number_format($tbuy,2));

  pdf_set_text_pos($pdf, get_right_pos(number_format($tsell,2), $pdf, "402"), 570-$offset3);
 ...
 function Stationery($printStationery = false) {
  ...
  pdf_set_text_pos($pdf, 75+$pos2, 700);
...
if($hhhh)  {
 ...
 //pdf_continue_text($pdf, $row[phone]);
  ...
  if($printStationery) {
   ...
   pdf_setfont($pdf, $font, 12);
   pdf_set_text_pos($pdf, get_right_pos($row['acno'].":", $pdf, "150"), 760);
   pdf_continue_text($pdf, $row['acno'].":");
   pdf_set_text_pos($pdf, get_right_pos($row['sdate'].":", $pdf, "150"), 735);
   ...
   pdf_set_text_pos($pdf, get_right_pos($row['sbuy'], $pdf, "343"), 605);
   pdf_continue_text($pdf, $row['sbuy']);

   pdf_set_text_pos($pdf, get_right_pos($row['ssell'], $pdf, "402"), 605);
   ...
   foreach($Data as $key => $value) {
    pdf_set_text_pos($pdf, get_right_pos($value, $pdf, "462"), 605-$DataCount);
