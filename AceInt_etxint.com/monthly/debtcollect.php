<?
function feeletters($memberarray,$ltype,$header)  {
...
//global $pdf, $pdfsig, $row, $pdfimage;
 ...
 while($row = mysql_fetch_assoc($query)) {
  ...
  $blah .= "$row2[memid],$reg,$hol,$row2[abn],$row2[streetno] $row2[streetname] $row2[suburb],$row2[city] $row2[state] $row2[postcode],$row2[postalno] $row2[postalname] $row2[postalsuburb],$row2[postalcity] $row2[postalstate] $row2[postalpostcode],$row2[homestreetno] $row2[homestreetname] $row2[homesuburb],$row2[homecity] $row2[homestate] $row2[homepostcode],$row2[phonearea] $row2[phoneno],$row2[faxarea] $row2[faxno],$row2[mobile],$row2[homephone],$row[dollarfees],$net,$claim,60 Days\r\n";
