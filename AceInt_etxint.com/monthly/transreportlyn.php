<?
 ...
 while($CountryRow = mysql_fetch_assoc($SQLQuery)) {
  ...
  // get the actual taxinvoice ready.
   ...
   $buffer2 = taxinvoice($CountryRow[countryID], true);
  ...
  // add the attachment on.
   $mail->add_attachment($buffer, 'transreport.pdf', 'application/pdf');
   //$mail->add_attachment($buffer2, 'transreportSmall.pdf', 'application/pdf');

  // build the message.
   $mail->build_message();

  // send the message.
   //$mail->send('Finance', 'finance@'.$CountryRow[countrycode].'.ebanctrade.com', 'E Banc Accounts - Head Office', 'accounts@ebanctrade.com', 'Transaction Report','Bcc: reports@ebanctrade.com');
   //$mail->send('Finance', 'dave@ebanctrade.com', 'E Banc Accounts - Head Office', 'accounts@ebanctrade.com', 'Transaction Report','Bcc: reports@ebanctrade.com');
    $subject = "Transaction Report";
     ...
     unset($addressArray);
   	...
   	$attachArray[] = array($buffer2, 'transreportSmall.pdf', 'base64', 'application/pdf');

	$addressArray[] = array(trim('finance@'.$CountryRow[countrycode].'.'. getWho($CountryRow[logo], 2)), 'Finance');

	sendEmail("accounts@" . $CountryRow[countrycode] .".". getWho($CountryRow[logo], 2), getWho($CountryRow[logo], 1) .' Accounts', 'accounts@' . $CountryRow[countrycode] . getWho($CountryRow[logo], 2), $subject, 'accounts@' . $CountryRow[countrycode] .'.' . getWho($CountryRow[logo], 2), getWho($CountryRow[logo], 1) .' Accounts', $text, $addressArray, $attachArray);

 }

function taxinvoice($CID = false, $inter = false) {
...
$query = dbRead("select * from area where CID='" .$CID. "'$extra order by place");
...
if($row['feepercent'] < 26) {
	$hh = " and feespaid.percent <> 50 ";
} else {
	$hh = "";
}

//$query3 = dbRead("select * from feespaid, members where (feespaid.memid=members.memid) and members.licensee='$row[FieldID]' and paymentdate like '$date2-%' order by paymentdate");
$query3 = dbRead("select * from feespaid, members where (feespaid.memid=members.memid) and feespaid.area='$row[FieldID]' and feespaid.type not in (6,7,8,9) and paymentdate like '$date2-%'$hh order by paymentdate, feespaid.transID");
...
pdf_lineto($pdf, 575, 750-$offset3);
pdf_stroke($pdf);
...
$query2 = dbRead("select count(memid) as signup from members where datejoined like '$date2-%' and area = $row[FieldID]");
$row2 = mysql_fetch_assoc($query2);

pdf_setfont($pdf, $font, 8);

pdf_set_text_pos($pdf, get_right_pos("Number of Signups: $row2[signup]", $pdf, "300", 8, $font), 765-$offset3);
pdf_continue_text($pdf, "Number of Signups: $row2[signup]");
...
pdf_set_text_pos($pdf, get_right_pos("Comm", $pdf, "575", 8, $font), 770-$offset3);
