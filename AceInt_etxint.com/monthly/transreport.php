<?
 ...
 $query = dbRead("select * from area where CID = 1 and `drop` = 'Y' order by place");
 ...
 while($row = mysql_fetch_assoc($query)) {
  ...
  // send the message.
   ...
   //$mail->send($row[tradeq], 'dave@ebanctarde.com', 'E Banc Accounts - Head Office', 'accounts@ebanctrade.com', 'Transaction Report - '.$row[place],'Bcc: reports@ebanctrade.com'.$otheremail);
	$subject = "Transaction Report - ".$row[place];
...
$query4 = dbRead("select * from area where FieldID = ".$run_fieldid);
...
$query3 = dbRead("select * from feespaid, members where (feespaid.memid=members.memid) and feespaid.area='$run_fieldid' and paymentdate like '$date2-%' and feespaid.type not in (6,7,8,9)$hh order by paymentdate, feespaid.transID");
