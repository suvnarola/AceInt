...
/** * Statement Print  */
function statement($query, $printStationery = true, $individual = false, $send = false, $senddate = false, $nomonths = false) {
...
//$query3 = dbRead("select transactions.*, members.*, tbl_members_companyinfo.Companyname as TOCompanyname from transactions, members left outer join tbl_members_companyinfo on (transactions.to_memid = tbl_members_companyinfo.memid AND transactions.dis_date BETWEEN tbl_members_companyinfo.datefrom AND tbl_members_companyinfo.dateto) where transactions.to_memid = members.memid and transactions.memid = $row[memid] and dis_date between '#$date1-01#' and '#$date2-31#' order by dis_date");
            ...
            //echo "select transactions.*, members.*, tbl_members_companyinfo.Companyname as TOCompanyname from transactions, members left outer join tbl_members_companyinfo on (transactions.to_memid = tbl_members_companyinfo.memid AND transactions.dis_date BETWEEN tbl_members_companyinfo.datefrom AND tbl_members_companyinfo.dateto) where transactions.to_memid = members.memid and transactions.memid = $row[memid] and dis_date between '#$date1-01#' and '#$date2-31#' order by dis_date";
            echo "<br/>";
            $query3 = dbRead("select transactions.*, members.*, tbl_members_companyinfo.Companyname as TOCompanyname from transactions inner join members on (transactions.to_memid = members.memid) left outer join tbl_members_companyinfo on (transactions.to_memid = tbl_members_companyinfo.memid AND transactions.dis_date BETWEEN tbl_members_companyinfo.datefrom AND tbl_members_companyinfo.dateto) where transactions.memid = " . $row['memid'] . " and dis_date between '2020-12-01#' and '2020-12-31' order by dis_date");
            echo "select transactions.*, members.*, tbl_members_companyinfo.Companyname as TOCompanyname from transactions inner join members on (transactions.to_memid = members.memid) left outer join tbl_members_companyinfo on (transactions.to_memid = tbl_members_companyinfo.memid AND transactions.dis_date BETWEEN tbl_members_companyinfo.datefrom AND tbl_members_companyinfo.dateto) where transactions.memid = " . $row['memid'] . " and dis_date between '2020-12-01#' and '2020-12-31' order by dis_date";
            echo "<br/>";
            
            $query4 = dbRead("select (sum(sell)-sum(buy)) as optrade, sum(dollarfees) as opfees from transactions where memid = " . $row['memid'] . " and dis_date < '2020-12-01' group by memid");
            
            echo "select (sum(sell)-sum(buy)) as optrade, sum(dollarfees) as opfees from transactions where memid = " . $row['memid'] . " and dis_date < '2020-12-01' group by memid";
...
//            $cashtotal = ($cashtotal + $row4['opfees']);
            $box5 = "$row[overduefees]";
            $cashtotal = ($cashtotal + $row['overduefees']);
...
//3 top lines in boxes  pdf_setlinewidth($pdf, 1.5);  
...
////$box8= $row['currency']."$facility";  //$box9= $row['currency']."$refacility";  //$box10= $row['currency']."$nett";  //$box11= $row['currency']."$cash"; 
            ...
            if ($row['CID'] == 12 || $row['CID'] == 15) {
//$offset3 = $offset3 + 90;   //if($offset3 > 485) {    //pdf_end_page($pdf);    //pdf_begin_page($pdf, 595, 842);    //$offset3=0;    //Stationery($printStationery);   //}   
...
////pdf_continue_text($pdf, " "); //pdf_continue_text($pdf, $row[phone]);   //pdf_set_text_pos($pdf, get_right_pos($row['locoff'].":", $pdf, "450"), 670);   //pdf_continue_text($pdf, $row['locoff'].":");  
    if ($printStationery) {
        pdf_setcolor($pdf, "fill", "rgb", 0, 0, 0, 0);
        pdf_setfont($pdf, $font, 8);
        ...
        if ($row['CID'] == 1) {
            //$row['address2'] = 'Po Box 1176, Hawksburn, Vic 3142';          //$row['email'] = 'hq@accesscommercial.exchange'; 
...
//          $pdfimage = pdf_open_image_file($pdf, "jpeg", "/home/etxint/admin.etxint.com/images/".$ima."-bw.jpg");   //pdf_place_image($pdf, $pdfimage, 445, 755, 1);	
        ...
        if ($row['logo'] == 'etx') {
            //$pdfimage = pdf_open_image_file($pdf, "jpeg", "/home/etxint/public_html/home/images/".$logo."2-bw.jpg");   		//pdf_place_image($pdf, $pdfimage, 455, 740, .25);    
            //$pdfimage = pdf_load_image($pdf, "jpeg", "/home/etxint/public_html/home/images/" . $logo . "2-bw.jpg", '');

            /* Commented by rep on 15-10-2018 and added below code */
...
//            $pdfimage = pdf_load_image($pdf, "jpeg", "/home/etxint/admin.etxint.com/images/ext-new-bw.jpg", '');
//            pdf_fit_image($pdf, $pdfimage, 455, 740, "scale 0.40");
        } else {     //$pdfimage = pdf_open_image_file($pdf, "jpeg", "/home/etxint/admin.etxint.com/images/".$logo."-bw.jpg");   		//pdf_place_image($pdf, $pdfimage, 445, 780, 1);    	
            $pdfimage = pdf_load_image($pdf, "jpeg", "/home/etxint/admin.etxint.com/images/" . $logo . "-bw.jpg", '');
