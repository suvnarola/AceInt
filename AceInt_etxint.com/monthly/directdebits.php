...
$query = dbRead("select * from members, invoice, tbl_admin_payment_types where (invoice.memid=members.memid) and (members.paymenttype = tbl_admin_payment_types.FieldID) and tbl_admin_payment_types.ddrun='1' and members.paymenttype = 20 and accountno > 0 and invoice.date='$date2' and (overduefees+currentpaid+currentfees)>'5' and members.CID = 1 order by accountname");
 ...
 while($row = mysql_fetch_assoc($query)) {
  ...
  if($totall > '500')  {
   ...
   $blah .= "$row[accountno],$row[accountname],$totall,Fee Payment- $row[memid] \r\n";
