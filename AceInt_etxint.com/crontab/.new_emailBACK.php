<?
 ...
 $Cquery = dbRead("select country.*, countrydata.* from country, countrydata where (country.countryID = countrydata.CID) and Display = 'Yes' and countryID not in (12,10,3,15)  and countryID = 1 order by countryID");
 ...
 //do emails for the individual countries first.
  ...
  $query3 = dbRead("SELECT registered_accounts.Acc_No FROM registered_accounts INNER JOIN reg_acc_details ON registered_accounts.Acc_No = reg_acc_details.Acc_No WHERE (((registered_accounts.Trade_Refund)>0)) GROUP BY registered_accounts.Acc_No","ebanc_services");

  while($row3 = mysql_fetch_array($query3)) {

  $query2 = dbRead("select members.memid as memid, members.companyname as companyname, tbl_members_email.email as emailaddress, members.contactname as contactname, invoice.* from invoice, members, status, tbl_members_email where invoice.memid=members.memid and (members.status = status.FieldID) and (members.memid = tbl_members_email.acc_no) and members.monthlyfeecash = '0' and invoice.date like '$invoice_date-%' and (status.mem_send_inv = 1) and tbl_members_email.email != '' and tbl_members_email.type = 2 and members.CID = '".$Crow['countryID']."' and invoice.memid = ".$row3['Acc_No']." order by companyname");
  ...
  if($row2[memid]) {
   ...
   //} elseif($Crow['countryID'] == 1) {
		...
		if(strstr($row2['emailaddress'], ";")) {
			$emailArray = explode(";", $row2['emailaddress']);
			foreach($emailArray as $key => $value) {
		   		$mail->send($row2['contactname'], trim($value), "Empire Trade Accounts", "accounts@".$Crow[countrycode].".empireXchange.com", "Amended ".$Crow['sname']." / ".$Crow['tname']." - ".$row2['companyname'],"Reply-To: TaxInvoiceQuery <accounts@".$Crow[countrycode].".empireXchange.com>\r\nErrors-To: TaxInvoiceEmailError <dave@empireXchange.com>\r\n");
   			}
   		} else {
			...
			sendEmail("accounts@".$Crow[countrycode].".empireXchange.com", "Empire Trade Accounts", "accounts@".$Crow[countrycode].".empireXchange.com", "Amended ".$Crow['sname']." / ".$Crow['tname']." - ".$row2['companyname'], "accounts@".$Crow[countrycode].".empireXchange.com", "Empire Trade Accounts", $text, $addressArray, $attachArray);
   		}
