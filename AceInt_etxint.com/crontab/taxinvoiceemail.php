...
function taxinvoice($run_memid,$run_date,$amount,$invno) {
 
 $query = dbRead("select * from country, countrydata, members where (((members.memid = '$run_memid') and (members.CID = country.countryID)) and (members.CID = countrydata.CID))");
 ...
 while($row = mysql_fetch_assoc($query)) {
  ...
  if ($row[abn] != 0 && $row[gst] = Y) {
   $gst=number_format((($amount/(100+$row[tax]))*$row[tax]),2);
   $nett=number_format($amount-$gst,2);
   $total=number_format($amount,2); 
  } else {
   $gst='0.00';
   $nett=number_format($amount,2);
   $total=number_format($amount,2);
  }
  ...
  pdf_show_boxed($pdf, $accountdetails, 440, 667, 85, 60, "left");
