<?
 ...
 while($ARow = mysql_fetch_assoc($SQLQuery)) {
  ...
  //$Query7 = dbRead("select sum(transactions.sell) as Facility, extract(year_month from transactions.dis_date) as date1 from transactions, members where (members.memid = transactions.memid) and members.area = '".$ARow['FieldID']."' and transactions.to_memid = ".$Country['facacc']." group by date1");
  //while($Row7 = mysql_fetch_assoc($Query7)) {
   
   //if($Row7['date1'] != 0) {
   
    //$YearArray[$Row7[date1]][$ARow['FieldID']]['GS_Facility'] = $Row7['Facility'];
 ...
 foreach($YearArray as $Key => $Value) {
  ...
  $ym = $year."-".$month."-".$day;
   ...
   $tt13 = $Row13['SumBuy'] - $Row13['SumSell'];
