...
<cfset filename="">
	...
	<cfif ((root_path is not "") and (upload_path is not "") and (enable_upload is "yes"))>
		<cfif ((pages_path is not "") and (Form.section is "Pages"))>
			<cfset section_path = pages_path>
			<cfset allowed_files = "\.(" & Replace(page_formats, ",", "|", "All") & ")$">
		<cfelseif ((images_path is not "") and (Form.section is "Images"))>
			<cfset section_path = images_path>
			<cfset allowed_files = "\.(" & Replace(image_formats, ",", "|", "All") & ")$">
		<cfelseif ((files_path is not "") and (Form.section is "Files"))>
			<cfset section_path = files_path>
			<cfset allowed_files = "\.(" & Replace(file_formats, ",", "|", "All") & ")$">
		</cfif>
...
<cfinclude template="hyperlinknotification.cfm.html" >
