...
<%
	...
	if (! root_path.equals("")) {
		if (! pages_path.equals("")) {
			File fh = new File(root_path + pages_path);
		...
		if (! files_path.equals("")) {
			File fh = new File(root_path + files_path);
			if (fh.exists() && fh.isDirectory()) {
				files = fh.list();
			}
		}
		...
		if (dh != null) {
			...
			for (int folder=0; folder<dh.length; folder++) {
				...
				if (Pattern.compile(hidden_paths, Pattern.CASE_INSENSITIVE).matcher("" + base_path + path + entry).find()) {
					...
					output = output + "hyperlinklistfilterMenu.add(menuitem++," + menu + ",'" + entry.replaceAll("'", "\\\\'") + "','javascript:openit(\\'" + section + "\\',\\'" + path + entry + "/" + "\\',\\'" + entry + "\\')','','','','imgfolder.gif');" + "\r\n";
