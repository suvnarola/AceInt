...
if(!$_REQUEST['top']) {
 ...
 if($_REQUEST['f'] == "companyname") {
  $fff = "(companyname like '%".encode_text2(addslashes(trim($_REQUEST['data'])))."%' or contactname like '%".encode_text2(addslashes(trim($_REQUEST['data'])))."%' or regname like '%".encode_text2(addslashes(trim($_REQUEST['data'])))."%' or accholder like '%".encode_text2(addslashes(trim($_REQUEST['data'])))."%') and ";
 } elseif($_REQUEST['f'] == "emailaddress") {
  $fff = "tbl_members_email.email like '%".encode_text2(addslashes(trim($_REQUEST['data'])))."%' and ";
 } elseif(!$_REQUEST['f']) {
  $fff = "";
 } else {
  $fff = "".addslashes($_REQUEST['f'])." like '%".encode_text2(addslashes(trim($_REQUEST['data'])))."%' and ";
 }
 ...
 if($_REQUEST['deactive']) {
  $blahblah2 = " status.FieldID != 1 ";
 } else {
  $blahblah2 = " status.FieldID < 9 ";
 }
 ...
 if($_REQUEST['catid'] && $_REQUEST['licareaid']) {
	...
	$dbgetdataout = dbRead("
  	...
  	select members.area, members.memid as memid, members.companyname as companyname, members.contactname as contactname, members.phoneno as phoneno, members.city as city, members.phonearea as phonearea, members.status as status, members.trade_per as trade_per, members.date_per as date_per, max(mem_categories.category) as cat1, mem_categories.description as descrip, members.letters as letters, members.bdriven as bdriven, members.t_unlist as t_unlist, members.CID as CID, members.fiftyclub, members.uncon as uncon, status.*
	  ...
	  left outer join tbl_members_email on (tbl_members_email.acc_no = members.memid)

	  where

	  	(mem_categories.category IN (".addslashes($_REQUEST['catid']).")) and
		members.licensee = '".addslashes($_REQUEST['licareaid'])."'	and
		$fff
		status.Type not like '$blahblah'$fifty $State and
		members.CID = $GET_CID
	...
	where
		...
		(mem_categories.category IN (".addslashes($_REQUEST['catid']).")) and
		((tbl_area_physical.FieldID = ".addslashes($_REQUEST['areaid']).") $nationalMem ) and
		$fff
		status.Type not like '$blahblah'$fifty $State and
		members.CID = $GET_CID
  ...
  if(is_array($_REQUEST['catid'])) {
   $CatComma = comma_seperate($_REQUEST['catid']);
  } else {
   $CatComma = $_REQUEST['catid'];
  }
  ...
  $dbgetdataout = dbRead("
	...
	where
		...
		(mem_categories.category IN (".addslashes($CatComma).")) and
		(($area_array) $nationalMem ) and
		$fff
		status.Type not like '$blahblah'$fifty $State and
		members.CID = $GET_CID
  ...
  $dbgetdataout = dbRead("
	...
	select members.memid as memid, members.companyname as companyname, members.contactname as contactname, members.phoneno as phoneno, members.city as city, members.phonearea as phonearea, members.status as status, members.trade_per as trade_per, members.date_per as date_per, max(mem_categories.category) as cat1, members.letters as letters, members.bdriven as bdriven, members.t_unlist as t_unlist, members.CID as CID, members.fiftyclub, members.uncon as uncon, status.*
  ...
  $dbgetdataout = dbRead("
	...
	select members.memid as memid, members.companyname as companyname, members.contactname as contactname, members.phoneno as phoneno, members.city as city, members.phonearea as phonearea, members.status as status, members.trade_per as trade_per, members.date_per as date_per, max(mem_categories.category) as cat1, members.letters as letters, members.bdriven as bdriven, members.t_unlist as t_unlist, members.CID as CID, members.fiftyclub, members.uncon as uncon, status.*
		...
		where
			...
			((tbl_area_physical.FieldID = ".addslashes($_REQUEST['areaid']).") $nationalMem ) and
			$fff  status.Type not like '$blahblah'$fifty $State and
			members.CID = $GET_CID
...
<td class="Border">
  ...
  <tr>
      ...
      <td bgcolor="#FFFFFF">
      <input type="text" name="data" id=data0 value="<?= get_all_added_characters(encode_text2(trim($_REQUEST['data']))) ?>" size="35" tabindex="1"></td>
      ...
      <td bgcolor="#FFFFFF">
        <select name="f">
          ...
          <option <? if ($_REQUEST['f'] == "mobile") { echo "selected "; } ?>value="mobile">Mobile Phone</option>
          <option <? if ($_REQUEST['f'] == "abn") { echo "selected "; } ?>value="abn">ABN</option>
        </select>
      </td>
  </tr>
  <tr>
...
<?if($_SESSION['Country']['club'] == 1) {?>
      <td height="30" bgcolor="#FFFFFF">&nbsp;</td>
      <td height="30" colspan="2" bgcolor="#FFFFFF" align="left"><?= $_SESSION['Country']['fiftyName'] ?> Members <input type="checkbox" name="fifty" <? if($_REQUEST['fifty']) { echo "checked "; } ?> value="OFF"></td>
<?} else {?>
      <td height="30" bgcolor="#FFFFFF">&nbsp;</td>
...
<?if($_SESSION['Country']['gold'] == 1) {?>
      <td height="30" bgcolor="#FFFFFF">&nbsp;</td>
      <td height="30" colspan="2" bgcolor="#FFFFFF" align="left"><?= $_SESSION['Country']['goldName'] ?> Members <input type="checkbox" name="gold" <? if($_REQUEST['gold']) { echo "checked "; } ?> value="OFF"></td>
<?} else {?>
      <td height="30" bgcolor="#FFFFFF">&nbsp;</td>
      <td height="30" colspan="2" bgcolor="#FFFFFF" align="left"></td>
<?}?>      </tr>
...
?>
		<tr id="<?= $row['memid'] ?>" bgcolor="<?= $bgcolor ?>" onmouseover="setPointer(this, <?= $row['memid'] ?>, 'over', '<?= $bgcolor ?>', '#BBBBBB', '#AAAAAA');" onmouseout="setPointer(this, <?= $row['memid'] ?>, 'out', '<?= $bgcolor ?>', '#BBBBBB', '#AAAAAA');" onmousedown="setPointer(this, <?= $row['memid'] ?>, 'click', '<?= $bgcolor ?>', '#BBBBBB', '#AAAAAA');">
			<td width="40"><? if(checkmodule("PrintCheque") && ($_SESSION['Country']['countryID'] == 3 || $_SESSION['Country']['countryID'] == 10)) { ?><a href="includes/printcheque.php?memid=<?= $row['memid'] ?>" class="nav"><? } ?><font color="<?= $frcolor ?>"><?if($row['letters'] == 6 || $row['uncon'] == 'Y') {?><b><?= $row['memid'] ?></b><?} else {?><?= $row['memid'] ?><?}?></font><? if(checkmodule("PrintCheque")) { ?></a><? } ?>&nbsp;</td>
			...
			<td width="92"><font color="<?= $frcolor ?>"><?= get_all_added_characters($row['city']) ?></font>&nbsp;</td>
			<td width="118"><font color="<?= $frcolor ?>"><?= get_all_added_characters($areano) ?> <?= get_all_added_characters($row['phoneno']) ?></font>&nbsp;</td>
		...
		<?//if($_REQUEST['catid']) {?>
		 <?if($_SESSION['User']['emcus'] == 1 || (!checkmodule("EditMemberLevel2") && !$_SESSION['User']['emsal'] == 1)) { ?>
		 <tr id="<?= $row['memid'] . "85478" ?>" bgcolor="<?= $bgcolor ?>" onmouseover="setPointer(this, <?= $row['memid'] . "85478" ?>, 'over', '<?= $bgcolor ?>', '#BBBBBB', '#AAAAAA');" onmouseout="setPointer(this, <?= $row['memid'] . "85478" ?>, 'out', '<?= $bgcolor ?>', '#BBBBBB', '#AAAAAA');" onmousedown="setPointer(this, <?= $row['memid'] . "85478" ?>, 'click', '<?= $bgcolor ?>', '#BBBBBB', '#AAAAAA');">
		  <td colspan="5"><font color="<?= $frcolor ?>"><?= get_all_added_characters($row['descrip']) ?></font>&nbsp;</td>
		  <td colspan="5" align="right"><?= $row['trade_per'] ?>% / <?= date("d-m-y", strtotime($row['date_per'])) ?>&nbsp;</td>
		 </tr>
		 <?}?>
		<?//}?>
<?
...
<td class="Border">
  ...
  <tr>
      ...
      <td bgcolor="#FFFFFF">
      <input type="text" name="data" id=data size="35" tabindex="1" value="<?= get_all_added_characters(encode_text2(trim($_REQUEST['data']))) ?>"></td>
...
<?if($_SESSION['Country']['club'] == 1) {?>
      <td height="30" bgcolor="#FFFFFF">&nbsp;</td>
      <td height="30" colspan="2" bgcolor="#FFFFFF" align="left"><?= $_SESSION['Country']['fiftyName'] ?> <input type="checkbox" name="fifty" <? if($_REQUEST['fifty']) { echo "checked "; } ?> value="OFF"></td>
<?} else {?>
      <td height="30" bgcolor="#FFFFFF">&nbsp;</td>
      <td height="30" colspan="2" bgcolor="#FFFFFF" align="left"></td>
<?}?>
...
<?if($_SESSION['Country']['gold'] == 1) {?>
      <td height="30" bgcolor="#FFFFFF">&nbsp;</td>
      <td height="30" colspan="2" bgcolor="#FFFFFF" align="left"><?= $_SESSION['Country']['goldName'] ?> <input type="checkbox" name="gold" <? if($_REQUEST['gold']) { echo "checked "; } ?> value="ON"></td>
<?} else {?>
      <td height="30" bgcolor="#FFFFFF">&nbsp;</td>
      <td height="30" colspan="2" bgcolor="#FFFFFF" align="left"></td>
<?}?>
...
<?
		...
		while($row = mysql_fetch_assoc($dbgetarea)) {
			?>
			<option <? if(!isset($_REQUEST['countryid']) && $_SESSION['User']['CID'] == $row['countryID']) { echo "selected "; } elseif($GET_CID == $row['countryID']) { echo "selected "; } ?>value="<?= $row['countryID'] ?>"><?= $row['name'] ?></option>
			<?
...
?>
  ...
  <tr>
      <td width="100" height="30" class="Heading2"></td>
      <td width="450" height="30" colspan="3" bgcolor="#FFFFFF"><input type="Submit" value="<?= get_word("48","1") ?>">&nbsp;<input type="Submit" name="emsa" value="<?= get_page_data("5","1") ?>">&nbsp;<input type="Submit" name="vssa" value="<?= get_page_data("6","1") ?>">&nbsp;<? if(checkmodule("PrintCheque")) { ?><input type="Submit" name="pcheque" value="<?= get_page_data("7","1") ?>"><? } ?></td>
  </tr>
...
<form name="search">
Search: <div id="matches-container"><div id="matches"></div></div><input type="text" name="q" id="q" size="40" autocomplete="off" /> <input type="submit" value="Go" />
</form>
<?

$lastFive = displayLastMembers();

?>
<table width="620" cellpadding="3" cellspacing="0">
	<tr>
		<td>Last 5 Members Viewed.</td>
	</tr>
	<tr>
		<td style="border: 1px solid #000000"><?
				...
				foreach($lastFive as $key => $value) {
					...
					?>
					<a href="/includes/redirect.php?data=<?= $value['memid'] ?>&emsa=true"">[<?= $key + 1 ?>]: <?= $value['memid'] ?> - <?= $value['companyname'] ?></a><br>
					<?
...
if($_SESSION['Country']['catgroups'] == 1) {
 include("mem_searchGroup.php");
}
