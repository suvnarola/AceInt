...
<form method="POST" action="abc9.php" name="FAC">
...
<table width="100%" border="0" cellpadding="3" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111">
	<tr>
		...
		<td class="Heading2" width="100" align="right"><b><?= get_word("50") ?>:<a href="directory_pre_sendTEMP.php?SendInfo=Normal" class="nav">PDF Document (<?= GetFileSize(strlen($BufferNormal)); ?>)</a></b></td>
...
//$query = dbRead("select tbl_area_regional.FieldID as FieldID, tbl_area_regional.RegionalName as place, CID as CID from tbl_area_regional where CID = 16");
 ...
 //while($row = mysql_fetch_assoc($query)) {
  ...
  // }
// }
    //dbWrite("insert into tbl_area_states (StateName,CID) values ('".addslashes(encode_text2($_REQUEST[memid]))."','9')");
    //dbWrite("insert into tbl_area_physical (AreaName,RegionalID,CID) values ('".addslashes(encode_text2($_REQUEST[memid]))."','661','9')");
    //dbWrite("insert into tbl_area_regional (RegionalName,StateID,CID) values ('".addslashes(encode_text2($_REQUEST[memid]))."','237','9')");
