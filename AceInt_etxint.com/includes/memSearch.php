...
if($_REQUEST['memsearch']) {
 ...
 if($_REQUEST['catid'] && $_REQUEST['licareaid']) {
  $dbgetdataout = dbRead("select members.memid as memid, members.companyname as companyname, members.contactname as contactname, members.phoneno as phoneno, members.city as city, members.phonearea as phonearea, members.status as status, max(mem_categories.category) as cat1, members.letters as letters, members.bdriven as bdriven, members.t_unlist as t_unlist, members.CID as CID, status.* from members, status$AreaTable left outer join mem_categories on (members.memid = mem_categories.memid) where $StateLink(members.status = status.FieldID) and (mem_categories.category IN (".addslashes($_REQUEST['catid']).")) and members.licensee = '".addslashes($_REQUEST['licareaid'])."' and $fff and status.Type not like '$blahblah' $State and members.CID like '$GET_CID' group by members.memid order by companyname ASC");
 } elseif($_REQUEST['catid'] && $_REQUEST['areaid']) {
  $dbgetdataout = dbRead("select members.memid as memid, members.companyname as companyname, members.contactname as contactname, members.phoneno as phoneno, members.city as city, members.phonearea as phonearea, members.status as status, max(mem_categories.category) as cat1, members.letters as letters, members.bdriven as bdriven, members.t_unlist as t_unlist, members.CID as CID, status.* from members, status$AreaTable left outer join mem_categories on (members.memid = mem_categories.memid) where $StateLink(members.status = status.FieldID) and (mem_categories.category IN (".addslashes($_REQUEST['catid']).")) and tbl_area_physical.FieldID = ".addslashes($_REQUEST['areaid'])." and $fff and status.Type not like '$blahblah' $State and members.CID like '$GET_CID' group by members.memid order by companyname ASC");
 } elseif($_REQUEST['catid'] && $_REQUEST['disareaid']) {
  $dbgetdataout = dbRead("select members.memid as memid, members.companyname as companyname, members.contactname as contactname, members.phoneno as phoneno, members.city as city, members.phonearea as phonearea, members.status as status, max(mem_categories.category) as cat1, members.letters as letters, members.bdriven as bdriven, members.t_unlist as t_unlist, members.CID as CID, status.* from members, status$AreaTable left outer join mem_categories on (members.memid = mem_categories.memid) where $StateLink(members.status = status.FieldID) and (mem_categories.category IN (".addslashes($_REQUEST['catid']).")) and ($area_array) and $fff and status.Type not like '$blahblah' $State and members.CID like '$GET_CID' group by members.memid order by companyname ASC");
 } elseif($_REQUEST['licareaid']) {
  //$dbgetdataout = dbRead("select members.memid as memid, members.companyname as companyname, members.contactname as contactname, members.phoneno as phoneno, members.city as city, members.phonearea as phonearea, members.status as status, max(mem_categories.category) as cat1, members.letters as letters, members.bdriven as bdriven, members.t_unlist as t_unlist, members.CID as CID, status.* from members, status$AreaTable left outer join mem_categories on (members.memid = mem_categories.memid) where $StateLink(members.status = status.FieldID) and $searcharea like '".addslashes($_REQUEST['areaid'])."' and $fff and status.Type not like '$blahblah' $State and members.CID like '$GET_CID' group by members.memid order by companyname ASC");
  $dbgetdataout = dbRead("select members.memid as memid, members.companyname as companyname, members.contactname as contactname, members.phoneno as phoneno, members.city as city, members.phonearea as phonearea, members.status as status, max(mem_categories.category) as cat1, members.letters as letters, members.bdriven as bdriven, members.t_unlist as t_unlist, members.CID as CID, status.* from members, status$AreaTable left outer join mem_categories on (members.memid = mem_categories.memid) where $StateLink(members.status = status.FieldID) and members.licensee = '".addslashes($_REQUEST['licareaid'])."' and $fff and status.Type not like '$blahblah' $State and members.CID like '$GET_CID' group by members.memid order by companyname ASC");
 } elseif($_REQUEST['areaid']) {
  //$dbgetdataout = dbRead("select members.memid as memid, members.companyname as companyname, members.contactname as contactname, members.phoneno as phoneno, members.city as city, members.phonearea as phonearea, members.status as status, max(mem_categories.category) as cat1, members.letters as letters, members.bdriven as bdriven, members.t_unlist as t_unlist, members.CID as CID, status.* from members, status$AreaTable left outer join mem_categories on (members.memid = mem_categories.memid) where $StateLink(members.status = status.FieldID) and $searcharea like '".addslashes($_REQUEST['areaid'])."' and $fff and status.Type not like '$blahblah' $State and members.CID like '$GET_CID' group by members.memid order by companyname ASC");
  $dbgetdataout = dbRead("select members.memid as memid, members.companyname as companyname, members.contactname as contactname, members.phoneno as phoneno, members.city as city, members.phonearea as phonearea, members.status as status, max(mem_categories.category) as cat1, members.letters as letters, members.bdriven as bdriven, members.t_unlist as t_unlist, members.CID as CID, status.* from members, status$AreaTable left outer join mem_categories on (members.memid = mem_categories.memid) where $StateLink(members.status = status.FieldID) and tbl_area_physical.FieldID = ".addslashes($_REQUEST['areaid'])." and $fff and status.Type not like '$blahblah' $State and members.CID like '$GET_CID' group by members.memid order by companyname ASC");
 } elseif($_REQUEST['disareaid']) {
  $dbgetdataout = dbRead("select members.memid as memid, members.companyname as companyname, members.contactname as contactname, members.phoneno as phoneno, members.city as city, members.phonearea as phonearea, members.status as status, max(mem_categories.category) as cat1, members.letters as letters, members.bdriven as bdriven, members.t_unlist as t_unlist, members.CID as CID, status.* from members, status$AreaTable left outer join mem_categories on (members.memid = mem_categories.memid) where $StateLink(members.status = status.FieldID) and ($area_array) and $fff and status.Type not like '$blahblah' $State and members.CID like '$GET_CID' group by members.memid order by companyname ASC");
 } elseif($_REQUEST['catid']) {
  $dbgetdataout = dbRead("select members.memid as memid, members.companyname as companyname, members.contactname as contactname, members.phoneno as phoneno, members.city as city, members.phonearea as phonearea, members.status as status, max(mem_categories.category) as cat1, members.letters as letters, members.bdriven as bdriven, members.t_unlist as t_unlist, members.CID as CID, status.* from members, status$AreaTable left outer join mem_categories on (members.memid = mem_categories.memid) where $StateLink(members.status = status.FieldID) and (mem_categories.category IN (".addslashes($_REQUEST['catid']).")) and $fff and status.Type not like '$blahblah' $State and members.CID like '$GET_CID' group by members.memid order by companyname ASC");
 } else {
  $dbgetdataout = dbRead("select members.memid as memid, members.companyname as companyname, members.contactname as contactname, members.phoneno as phoneno, members.city as city, members.phonearea as phonearea, members.status as status, max(mem_categories.category) as cat1, members.letters as letters, members.bdriven as bdriven, members.t_unlist as t_unlist, members.CID as CID, status.* from members, status$AreaTable left outer join mem_categories on (members.memid = mem_categories.memid) where $StateLink(members.status = status.FieldID) and $fff and status.Type not like '$blahblah' $State and members.CID like '$GET_CID' group by members.memid order by companyname ASC");
 }
...
<?if($_SESSION['Country']['countryID'] == 1) {?>	  
      <td height="30" bgcolor="#FFFFFF">&nbsp;</td>
      <td height="30" colspan="2" bgcolor="#FFFFFF" align="left"><?= get_page_data("2") ?><input type="checkbox" name="fifty" <? if($_REQUEST['deactive']) { echo "checked "; } ?> value="OFF"></td>
<?} else {?>
      <td height="30" bgcolor="#FFFFFF">&nbsp;</td>
...
<?if($_SESSION['Country']['countryID'] != 3 && $_SESSION['Country']['countryID'] != 12 && $_SESSION['Country']['countryID'] != 2) {?>  
    <tr>
...
<table width="620" border="0" cellpadding="4" cellspacing="0">
	<tr>
		<td colspan="9" align="center" class="Heading"><?= get_page_data("8") ?>.</td>
	</tr>
	<tr>
...
?>
		<tr id="<?= $row['memid'] ?>" bgcolor="<?= $bgcolor ?>" onmouseover="setPointer(this, <?= $row['memid'] ?>, 'over', '<?= $bgcolor ?>', '#BBBBBB', '#AAAAAA');" onmouseout="setPointer(this, <?= $row['memid'] ?>, 'out', '<?= $bgcolor ?>', '#BBBBBB', '#AAAAAA');" onmousedown="setPointer(this, <?= $row['memid'] ?>, 'click', '<?= $bgcolor ?>', '#BBBBBB', '#AAAAAA');">
			...
			<td width="16" align="right"><a href="javascript:notes('body.php?page=notes&memid=<?= $row['memid'] ?>')"><img src="images/icon_n<? if($notesarray[$row['memid']] == 1) { echo "2"; } ?>.gif" border="0" width="16" height="15"></a></td>
			<td width="16" align="right"><a href="/includes/redirect.php?data=<?= $row['memid'] ?>&emsa=true"><img src="images/icon_e.gif" border="0" width="16" height="15"></a></td>
			<td width="16" align="left" valign="middle"><a href="body.php?page=member_edit&Client=<?= $row['memid'] ?>&pageno=1&tab=tab7&currentmonth=<?= date("n") ?>&numbermonths=1&currentyear=<?= date("Y") ?>&DisplayStatement=1"><img src="images/icon_s.gif" border="0" width="16" height="15"></a></td>
			<td width="16" align="left" valign="middle"><b><?= $let ?><?= $let2 ?></b></td>
		</tr>
<?
...
?>
      ...
      <td height="30" bgcolor="#FFFFFF">&nbsp;</td>
      <td height="30" bgcolor="#FFFFFF" align="left"></td>
    </tr>
<?if($_SESSION['Country']['countryID'] != 3 && $_SESSION['Country']['countryID'] != 12 && $_SESSION['Country']['countryID'] != 2) {?>      
    <tr>
