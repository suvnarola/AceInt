<?
 ...
 if($_REQUEST['select'] || $_REQUEST['refresh'])  {
  ...
  <td class="Border">
   <table width="100%" border="0" cellpadding="3" cellspacing="0">
	...
	<tr>
		...
		<td width="150" class="Heading2"><b><?= get_word("3") ?>:</b></font></td>
		<td width="250" class="Heading2"><b></b></font></td>
		<td width="60" class="Heading2"><b></b></font></td>
		<td width="50" class="Heading2"><b></b></font></td>
		<td width="20" class="Heading2">&nbsp;</td>
	</tr>
...
?>
		<tr bgcolor="<?= $bgcolor ?>">
			<td width="40"><a href="body.php?page=member_edit&Client=<?= $row['memid'] ?>&pagno=1&tab=tab5 " class="nav" target="_blank"><?= $row['memid'] ?></a></td>
			<td width="150"><a href="body.php?page=member_edit&Client=<?= $row['memid'] ?>&pagno=1&tab=tab5 " class="nav" target="_blank"><?= get_all_added_characters($row['companyname']) ?></a><br><font size="1"> - <?= get_all_added_characters($row['place']) ?> - </font><?if($row['fiftyclub'] == 1) {?><br><b> - <?= get_word(226)?> -</b><?}?><?if($row['fiftyclub'] == 2) {?><br><b> - <?= get_word(227)?> -</b><?}?></td>
			<td width="250"><?= get_all_added_characters($row['description']) ?></td>
			<td width="60" align="right"><?= get_all_added_characters($row['city']) ?><br><?= get_all_added_characters($row['state']) ?></td>
			<td align="right" width="100"><?= get_all_added_characters($row['category']) ?><br><?= date("d-m-y", strtotime($row['date_per'])) ?><br><?= $row['trade_per'] ?>% </td>
			<td width="20"><input type="checkbox" name="id2[]" value="<?= $row['FieldID'] ?>" checked></td>
		</tr>
<?
$foo++;
}
?>
	<tr>
		<td colspan="6" align="right" bgcolor="#FFFFFF"> <input type="submit" value="Refresh" name="refresh"> <input type="submit" value="Continue" name="checktransactions">
</td>
	</tr>
   </table>
  </td>
...
<?
       ...
       <tr>
        ...
        <td bgcolor="#FFFFFF" width="100%">
         <table border="0" cellpadding="3" cellspacing="0" align="left" width="100%">
          ...
          <tr>
           <td colspan="2"><input type="text" size="24" name="EmailAddress" style="font-size: 8pt" value="<?= $_SESSION['Admin']['email'] ?>">&nbsp;<input style="font-size: 8pt;" type="Submit" name="SendEmail" value="<?= get_page_data("4") ?>">&nbsp;<br><input type="checkbox" value="ON" checked name="EmailUncompressed">&nbsp;<?= get_page_data("5") ?>.<br><input type="checkbox" value="ON" <?if($_SESSION['Directory']['id2']) {?>checked<?}?> name="bcc">&nbsp;Send me a copy.</td>
          </tr>
          <tr>
 ...
 function trans_session() {
  ...
  $_SESSION['Directory']['area2']     = (!empty($_SESSION['Directory']['area2']) && $_SESSION['Directory']['area2'] == $_REQUEST['area2'])             ? $_SESSION['Directory']['area2']     : $_REQUEST['area2'];
