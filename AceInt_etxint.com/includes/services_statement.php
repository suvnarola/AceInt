...
<form method="POST" action="body.php?page=services_statement&ID=<?= $_GET[ID] ?>&tab=<?= $_GET[tab] ?>" name="frmletters">
...
$dbgettrans = dbRead("select * from  registered_accounts, transactions where (registered_accounts.FieldID = transactions.regAccID)$yy order by transactions.dis_date, transactions.id ASC",ebanc_services);
