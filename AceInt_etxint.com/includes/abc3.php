...
if($_REQUEST[memid])  { 
 ...
 //dbWrite("insert into tbl_area_states (StateName,CID) values ('".addslashes(encode_text2($_REQUEST[memid]))."','12')");
  //$gg = dbWrite("insert into tbl_area_regional (RegionalName,StateID,CID) values ('".addslashes(encode_text2($_REQUEST[memid]))."','233','12')","etradebanc", TRUE);
  //dbWrite("insert into tbl_area_physical (AreaName,RegionalID,CID) values ('".addslashes(encode_text2($_REQUEST[memid]))."','".$gg."','12')");
 //$query = dbRead("select tbl_area_physical.FieldID as FieldID, tbl_area_physical.AreaName as place, CID as CID from tbl_area_physical where CID = 16");
 ...
 while($row = mysql_fetch_assoc($query)) {
    ...
    //dbWrite("update tbl_area_states set StateName = '".addslashes(encode_text2($_REQUEST['memid']))."' where FieldID=171");
    //dbWrite("update tbl_area_regional set RegionalName = '".addslashes(encode_text2($_REQUEST['memid']))."' where FieldID=649");
    //dbWrite("update tbl_area_physical set AreaName = '".addslashes(encode_text2($row['place']))."' where FieldID=".$row['FieldID']."");
    dbWrite("update tbl_area_regional set RegionalName = '".addslashes(encode_text2($row['place']))."' where FieldID=".$row['FieldID']."");
