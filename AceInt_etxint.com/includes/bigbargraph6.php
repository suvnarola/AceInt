...
// Bar Graph
 ...
 $_REQUEST['Country'] = ($_REQUEST['Country']) ? $_REQUEST['Country'] : "%";

// set the variables for each graph.

 $query = dbRead("select count(memid) as DB_Data, extract(year_month from members.datejoined) as DB_Data2 from members where CID like '".$_REQUEST['Country']."' group by DB_Data2 DESC limit 1,12");
 $query3 = dbRead("select count(memid) as DB_Data, extract(year_month from members.datejoined) as DB_Data2 from members where CID like '".$_REQUEST['Country']."' group by DB_Data2 DESC limit 13,100000000");
 $query2 = dbRead("select * from country where countryID = '".$_REQUEST['Country']."'");
 $row2 = mysql_fetch_assoc($query2);

 $EGRAPH['title'] = "Member Growth Projection - ".$_REQUEST['graphdata']." Month Average - ".$_REQUEST['graphdata2']." Month Projection - ".$row2['name'];
...
// get average members growth per month based on the previous months starting from last month.
 ...
 $average_membergrowth = number_format(array_sum($average_array)/$_REQUEST['graphdata']);
 
 $count = $_REQUEST['graphdata2']-1;
 $i = 0;
 for ($i = 0; $i <= $count; $i++) {
...
// make the month array.
 ...
 $count = 11 + $_REQUEST['graphdata2'];
 $i = 0;
 for ($i = 0; $i <= $count; $i++) {

  $datax[] = date("M y",mktime(1,1,1,(date("n")-$count-1+$_REQUEST['graphdata2'])+$i,1,date("Y")));
