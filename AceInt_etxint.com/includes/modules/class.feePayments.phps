	...
	class feePayment extends ebancSuite {
		...
		function ccPayment($memID, $ccAmount, $ccNO, $ccExpiryM, $ccExpiryY, $ccCVV, $ccType, $poNumber, $testServer = false) {
			...
			if($testServer) {
				...
				$ccPay = new SecurePay("testServer");
		...
		function addCCTrans($memID) {
		
			global $ebancAdmin;
		
			$poNumber = $ebancAdmin->dbWrite("insert into " . DB_TABLE_CREDIT . " (memid,date,userid) values ('" . $memID . "',now(),'" . $_SESSION['User']['FieldID'] . "')", DB_NAME, 1);
		...
		function feeReversal($memberRow, $transType, $feeAmount, $unHonourArray, $transDetails = false) {
			...
			switch($transType) {
				...
				case "3":
					...
					foreach($unHonourArray as $key => $value) {
						...
						while($sqlRow = mysql_fetch_assoc($sqlQuery)) {
							...
							if($payAmount > 0) {
								...
								$areaSQL = dbRead("select area.* from area where FieldID = " . $sqlRow['to_licensee']);
								$areaRow = mysql_fetch_assoc($areaSQL);
    							...
    							if($sqlRow['fee_paid'] > $payAmount) {
    								...
    								$feesPaidIDArray[] = $ebancAdmin->dbWrite("insert into " . DB_TABLE_FEESPAID . " (memid,paymentdate,amountpaid,deducted_fees,percent,area,type,feesincurrid) values ('" . $sqlRow['to_memid'] . "','" . date("Y-m-d") . "','-" . $payAmount . "','0','" . $chargePercent . "','" . $sqlRow['to_licensee'] . "','4','" . $sqlRow['fieldid'] . "')", DB_NAME, true);
    								...
    								$feesPaidIDArray[] = $ebancAdmin->dbWrite("insert into " . DB_TABLE_FEESPAID . " (memid,paymentdate,amountpaid,deducted_fees,percent,area,type,feesincurrid) values ('" . $sqlRow['to_memid'] . "','" . date("Y-m-d") . "','-" . $sqlRow['fee_paid'] . "','0','" . $chargePercent . "','" . $sqlRow['to_licensee'] . "','4','" . $sqlRow['fieldid'] . "')", DB_NAME, true);
					...
					$ebancAdmin->authNo();
					$transID = $ebancAdmin->dbWrite("insert into " . DB_TABLE_TRANSACTIONS . " (memid,date,to_memid,buy,sell,dollarfees,type,details,authno,dis_date,userid) values('" . $memberRow['memid'] . "', '" . mktime() . "','" . $_SESSION['Country']['reserveacc'] . "','0','0','" . abs($total) . "','10','Unhonour Reversal','" . $ebancAdmin->authNo . "','" . date("Y-m-d") . "',".$_SESSION['User']['FieldID'].")", DB_NAME, TRUE);
		...
		function addFees($transID, $fromMemberRow, $toMemberRow, $feeAmount, $transDetails) {
			...
			if($transID) {
				...
				$ebancAdmin->dbWrite("insert into " . DB_TABLE_FEESINCURRED . " (date,memid,licensee,to_memid,to_licensee,trans_id,fee_amount) values ('".date("Y-m-d")."','".$fromMemberRow['memid']."','".$fromMemberRow['licensee']."','".$toMemberRow['memid']."','".$toMemberRow['licensee']."','".$transID."','".$feeAmount."')", DB_NAME);
		...
		function payFees($memberRow, $feeAmount, $transType, $methodType, $paidTrade = false, $otherMemberRow = false, $transDetails = false) {

			global $ebancAdmin;

			$transDetails = ($transDetails) ? $transDetails : "Cash Fee Payment";
					...
					switch($transType) {
						...
						case "1":
							...
							if(!$paidTrade) {
								...
								$feesPaidIDArray[] = $ebancAdmin->dbWrite("insert into " . DB_TABLE_FEESPAID . " (memid,paymentdate,amountpaid,deducted_fees,percent,area,type,feesincurrid) values ('" . $toMemberRow['memid'] . "','" . date("Y-m-d") . "','" . $newAmount . "','0','" . $chargePercent . "','" . $toMemberRow['licensee'] . "','4','" . $sqlRow['fieldid'] . "')", DB_NAME, true);
						...
						switch($transType) {
							...
							case "1":
								...
								if(!$paidTrade) {
									...
									$feesPaidIDArray[] = $ebancAdmin->dbWrite("insert into " . DB_TABLE_FEESPAID . " (memid,paymentdate,amountpaid,deducted_fees,percent,area,type,feesincurrid) values ('" . $toMemberRow['memid'] . "','" . date("Y-m-d") . "','" . $payAmount . "','0','" . $chargePercent . "','" . $toMemberRow['licensee'] . "','4','" . $sqlRow['fieldid'] . "')", DB_NAME, true);
			...
			if(!$paidTrade) {
				...
				if($transType == 2) {
					...
					$feesPaidIDArray[] = $ebancAdmin->dbWrite("insert into " . DB_TABLE_FEESPAID . " (memid,paymentdate,amountpaid,deducted_fees,percent,area,type,feesincurrid) values ('" . $otherMemberRow['memid'] . "','" . date("Y-m-d") . "','" . $feeAmount . "','0','" . $chargePercent . "','" . $otherMemberRow['licensee'] . "','3','" . $sqlRow['fieldid'] . "')", DB_NAME, true);
