...
function taxinvoice($query, $stationery = true, $individual = false, $send = false, $rollover = false) {
...
//pdf_set_parameter($pdf, "license", "L500102-010000-105258-97D9C0");
    ...
    $font = PDF_load_font($pdf, "Verdana", "winansi", false);
    $fontbold = PDF_load_font($pdf, "VerdanaBold", "winansi", false);
//$fontbold = pdf_findfont($pdf, "Verdana", "winansi", 0);
...
//pdf_set_text_pos($pdf, get_left_pos($Line, $pdf, "70", 18, $font), 800);
                pdf_set_text_pos($pdf, 70, 800);
                pdf_continue_text($pdf, $row['tname']);

                pdf_setfont($pdf, $font, 12);

                pdf_set_text_pos($pdf, get_right_pos("$row[acno]:", $pdf, "435", 12, $font), 722);
                pdf_continue_text($pdf, "$row[acno]:");
                pdf_set_text_pos($pdf, get_right_pos("$row[tdate]:", $pdf, "435", 12, $font), 700);
                pdf_continue_text($pdf, "$row[tdate]:");
                pdf_set_text_pos($pdf, get_right_pos("$row[tno]:", $pdf, "435", 12, $font), 677);
...
//lines and boxes
                ...
                pdf_set_text_pos($pdf, get_right_pos("$row[tsub]:", $pdf, "410", 12, $font), 507);
                pdf_continue_text($pdf, "$row[tsub]:");

                if ($row[tax] != 0) {
                    pdf_set_text_pos($pdf, get_right_pos("$row[tgst]:", $pdf, "410", 12, $font), 482);
                    pdf_continue_text($pdf, "$row[tgst]:");
                }

                pdf_set_text_pos($pdf, get_right_pos("$row[ttot]:", $pdf, "410", 12, $font), 457);
                pdf_continue_text($pdf, "$row[ttot]:");

                pdf_setfont($pdf, $font, 10);
                pdf_set_text_pos($pdf, get_right_pos("$row[tout]:", $pdf, "410", 10, $font), 428);
                pdf_continue_text($pdf, "$row[tout]:");
                pdf_set_text_pos($pdf, get_right_pos("$row[tpaid]:", $pdf, "410", 10, $font), 398);
                pdf_continue_text($pdf, "$row[tpaid]:");
                pdf_set_text_pos($pdf, get_right_pos("$row[tnow]:", $pdf, "410", 10, $font), 368);
                pdf_continue_text($pdf, "$row[tnow]:");

                $pos = 295;
                $Newtrequ = explode("|", wordwrap($row[trequ], 92, "|"));
                foreach ($Newtrequ as $Line) {
                    pdf_set_text_pos($pdf, get_left_pos($Line, $pdf, "302.5", 10, $font), $pos);
                ...
                foreach ($Newtnom as $Line) {
                    pdf_set_text_pos($pdf, get_left_pos($Line, $pdf, "302.5", 9, $font), $pos);
                    pdf_continue_text($pdf, $Line);
                    $pos = $pos - 9;
                }
...
//		$Newtnom = explode("|", wordwrap($row[auth], 92, "|"));
                $Newtnom = explode("|", wordwrap("Access Commercial Exchange Pty Ltd is the national agent for ETX International Exchange Trust", 92, "|"));
                foreach ($Newtnom as $Line) {
                    pdf_set_text_pos($pdf, get_left_pos($Line, $pdf, "302.5", 9, $font), $pos);
                    pdf_continue_text($pdf, $Line);
                    $pos = $pos - 9;
...
//		$Newitt = explode("|", wordwrap($row[itt], 67, "|"));
                $Newitt = explode("|", wordwrap("ALL MEMBERS ARE REQUIRED TO REFER TO THEIR LEGAL OBLIGATIONS IN THE MEMBERSHIP RULES ON THE EXCHANGE'S WEBSITES.", 67, "|"));
                ...
                pdf_set_text_pos($pdf, get_right_pos("http://" . $web . "", $pdf, "553", 9, $font), 124);
                ...
                pdf_setfont($pdf, $font, 12);
                pdf_set_text_pos($pdf, get_left_pos($row['trem'], $pdf, "297.5", 12, $font), 100);
                pdf_continue_text($pdf, $row[trem]);

                pdf_setfont($pdf, $font, 10);
                pdf_set_text_pos($pdf, get_right_pos("$row[comna]:", $pdf, "145", 10, $font), 78);
                pdf_continue_text($pdf, "$row[comna]:");
                pdf_set_text_pos($pdf, get_right_pos("$row[acno]:", $pdf, "145", 10, $font), 38);
                pdf_continue_text($pdf, "$row[acno]:");
                pdf_set_text_pos($pdf, get_right_pos("$row[tnow]:", $pdf, "470", 10, $font), 78);
                pdf_continue_text($pdf, "$row[tnow]:");
                pdf_set_text_pos($pdf, get_right_pos("$row[tampa]:", $pdf, "470", 10, $font), 38);
                pdf_continue_text($pdf, "$row[tampa]:");
            } else {

                pdf_setfont($pdf, $font, 9);
                pdf_set_text_pos($pdf, get_left_pos("        XXXXXXXXXXXX", $pdf, "297.5", 9, $font), 163);
                pdf_continue_text($pdf, "        XXXXXXXXXXXX");

                pdf_setfont($pdf, $font, 9);
                pdf_set_text_pos($pdf, get_left_pos("13 110 102 648", $pdf, "395", 9, $font), 163);
...
//$total=number_format(($row[currentfees]+$row[overduefees])+$row[currentpaid],2);
            //$total = number_format(($row['currentfees'] + $row['overduefees'] + $row['currentpaid']), 2);
            $total = number_format(($row['over_payment'] + $row['monthlyfeecash'] + $row['overduefees']), 2);
...
//$fees="$row[overduefees]\n\n\n$row[currentpaid]\n\n\n$total";
            ...
            foreach ($Newdet as $Line) {
                pdf_set_text_pos($pdf, get_left_pos($Line, $pdf, "225", 7, $font), $pos);
                pdf_continue_text($pdf, $Line);
                $pos = $pos - 10;
            }
            ...
            if ($row[tax] != 0) {
                pdf_set_text_pos($pdf, get_right_pos($gst, $pdf, "520", 10, $font), 480);
                pdf_continue_text($pdf, $gst);
            }

            pdf_set_text_pos($pdf, get_right_pos($nett, $pdf, "520", 10, $font), 581);
            pdf_continue_text($pdf, $nett);
            pdf_set_text_pos($pdf, get_right_pos($nett, $pdf, "520", 10, $font), 505);
            pdf_continue_text($pdf, $nett);
            pdf_set_text_pos($pdf, get_right_pos($row['currentfees'], $pdf, "520", 10, $font), 458);
            pdf_continue_text($pdf, $row['currentfees']);

            pdf_set_text_pos($pdf, get_right_pos($row['overduefees'], $pdf, "520", 10, $font), 425);
            pdf_continue_text($pdf, $row['overduefees']);
            pdf_set_text_pos($pdf, get_right_pos($currentpaid, $pdf, "520", 10, $font), 401.5);
            pdf_continue_text($pdf, $currentpaid);
            pdf_set_text_pos($pdf, get_right_pos($total, $pdf, "520", 10, $font), 368);
            ...
            if ($row['CID'] == 1) {
                ...
                pdf_setfont($pdf, $font, 10);
                pdf_set_text_pos($pdf, get_left_pos("PAYMENT METHODS", $pdf, "165", 10, $font), 465);
...
//pdf_continue_text($pdf, $row[companyname]);
            ...
            pdf_set_text_pos($pdf, get_left_pos($row['tlate'], $pdf, "302.5", 12, $font), 227);
...
//pdf_fit_image($pdf, $pdfimage, 450, 720, "scale 0.35");
                    $pdfimage = pdf_load_image($pdf, "jpeg", "/home/etxint/admin.etxint.com/images/ext-new-bw.jpg", '');
                    pdf_fit_image($pdf, $pdfimage, 430, 720, "scale 0.50");
                } else {
//$pdfimage = pdf_open_image_file($pdf, "jpeg", "/home/etxint/admin.etxint.com/images/".$logo."-bw.jpg");
//pdf_place_image($pdf, $pdfimage, 445, 780, 1);
                    $pdfimage = pdf_load_image($pdf, "jpeg", "/home/etxint/admin.etxint.com/images/" . $logo . "-bw.jpg", '');
                    pdf_fit_image($pdf, $pdfimage, 445, 780, '');
                }
...
//pdf_setcolor($pdf, "fill", "rgb", 0, 0, 1);
                ...
                pdf_set_text_pos($pdf, get_right_pos("$row[acno]:", $pdf, "170", 12, $font), 775 - $pos3);
                pdf_continue_text($pdf, "$row[acno]:");
                pdf_set_text_pos($pdf, get_right_pos("$row[tdate]:", $pdf, "170", 12, $font), 750 - $pos3);
                pdf_continue_text($pdf, "$row[tdate]:");
                pdf_set_text_pos($pdf, get_right_pos("$row[tno]:", $pdf, "170", 12, $font), 727 - $pos3);
                pdf_continue_text($pdf, "$row[tno]:");
...
//if(!$_REQUEST['invoice'] || $row['inv_type'] == 1) {
                ...
                if ($rollover) {
                    $Newtrequ = explode("|", wordwrap("Payment is required within 7 days.", 45, "|"));
                } else {
                    $Newtrequ = explode("|", wordwrap($row['trequ'], 45, "|"));
                }

                foreach ($Newtrequ as $Line) {
//pdf_set_text_pos($pdf, get_left_pos($Line, $pdf, "302.5", 10, $font), $pos);
                    pdf_set_text_pos($pdf, get_left_pos(trim($Line), $pdf, "400", 10, $font), $pos);
                    pdf_continue_text($pdf, trim($Line));
                    $pos = $pos - 10;
                }
...
//$pos = $pos - 10;
                    ...
                    $Newtnom = explode("|", wordwrap($row['tnom'], 40, "|"));
                    foreach ($Newtnom as $Line) {
//pdf_set_text_pos($pdf, get_left_pos($Line, $pdf, "302.5", 10, $font), $pos);
                        pdf_set_text_pos($pdf, get_left_pos(trim($Line), $pdf, "400", 9, $font), $pos);
                        pdf_continue_text($pdf, $Line);
                        $pos = $pos - 9;
                    }
                }
                ...
                $Newtnom = explode("|", wordwrap($row['tlate'], 40, "|"));
                foreach ($Newtnom as $Line) {
//pdf_set_text_pos($pdf, get_left_pos($Line, $pdf, "302.5", 10, $font), $pos);
                    pdf_set_text_pos($pdf, get_left_pos(trim($Line), $pdf, "400", 9, $font), $pos);
                    pdf_continue_text($pdf, $Line);
...
//	   $Newitt = explode("|", wordwrap($row[itt], 67, "|"));
                ...
                pdf_set_text_pos($pdf, get_right_pos($row['company'], $pdf, "565", 8, $font), 715);
                pdf_continue_text($pdf, $row['company']);

                if ($row['CID'] == 1) {
//$row['address2'] = 'Po Box 1176, Hawksburn, Vic 3142';
//$row['email'] = 'hq@accesscommercial.exchange';
                    $web = 'accesscommercial.exchange';
                }
                $theight = 0;
                $text = explode(",", $row['address2'], 2);
                foreach ($text as $Line) {
                    $new_st = str_replace("1176","1198",$Line);
                    pdf_set_text_pos($pdf, get_right_pos(trim($Line), $pdf, "565", 8, $font), 705 - $theight);
                    pdf_continue_text($pdf, trim($new_st));
                    $theight += 9;
                }

                pdf_set_text_pos($pdf, get_right_pos("T: " . $row['phone'], $pdf, "565", 8, $font), 675);
                pdf_continue_text($pdf, "T: " . $row['phone']);
//	   pdf_set_text_pos($pdf, get_right_pos("F: ".$row['fax'], $pdf, "565", 8, $font), 665);
//	   pdf_continue_text($pdf, "F: ".$row['fax']);
                pdf_set_text_pos($pdf, get_right_pos("E: " . $row['email'], $pdf, "565", 8, $font), 665);
                pdf_continue_text($pdf, "E: " . $row['email']);
                pdf_set_text_pos($pdf, get_right_pos("W: " . $web . "", $pdf, "565", 8, $font), 655);
                pdf_continue_text($pdf, "W: " . $web . "");

                if ($row['CID'] != 15) {
                    pdf_set_text_pos($pdf, get_right_pos($row['abn'], $pdf, "565", 8, $font), 645);
                    pdf_continue_text($pdf, $row['abn']);
                }
...
//pdf_setcolor($pdf, "fill", "rgb", 0, 0, 1);
                ...
                if (!isset($_REQUEST['invoice']) || $row['inv_type'] == 1) {
                    pdf_setfont($pdf, $font, 14);
                    pdf_set_text_pos($pdf, 40, 105);
//pdf_set_text_pos($pdf, get_left_pos($row[trem], $pdf, "60", 14, $font), 105);
                    pdf_continue_text($pdf, $row['trem']);


                    pdf_setfont($pdf, $font, 10);
                    pdf_set_text_pos($pdf, get_right_pos("$row[comna]:", $pdf, "355", 10, $font), 90);
                    pdf_continue_text($pdf, "$row[comna]:");
                    pdf_set_text_pos($pdf, get_right_pos("$row[acno]:", $pdf, "355", 10, $font), 70);
                    pdf_continue_text($pdf, "$row[acno]:");
                    pdf_set_text_pos($pdf, get_right_pos("$row[tnow]:", $pdf, "355", 10, $font), 50);
                    pdf_continue_text($pdf, "$row[tnow]:");
                    pdf_set_text_pos($pdf, get_right_pos("$row[tampa]:", $pdf, "355", 10, $font), 30);
                    pdf_continue_text($pdf, "$row[tampa]:");
                }
            }
...
//	 $addressbox="$row[contactname]\n$row[companyname]\n$streetno$row[postalname]\n$suburb$city";
            if (isset($streetno) && isset($suburb))
                $addressbox = $row['contactname'] . '\n' . $row['companyname'] . '\n' . $streetno . $row['postalname'] . '\n' . $suburb . $city;
            elseif (!isset($streetno) && isset($suburb))
                $addressbox = $row['contactname'] . '\n' . $row['companyname'] . '\n' . $row['postalname'] . '\n' . $suburb . $city;
            elseif (isset($streetno) && !isset($suburb))
                $addressbox = $row['contactname'] . '\n' . $row['companyname'] . '\n' . $streetno . $row['postalname'] . '\n' . $city;
            else
            ...
            if ($row['CID'] == 15 || isset($_REQUEST['invoice'])) {
            ...
            $total = decimal_format(($row['over_payment'] + $row['monthlyfeecash'] + $row['overduefees']), 1, '', '', $row['CID']);
            //$row[overduefees]=number_format($row[overduefees],2);
            ...
            if (isset($invRow) && isset($invRow['inv_no']) && $invRow['inv_no']) {
                $inv_no = $invRow['inv_no'];
            } else {
                $inv_no = $row['FieldID'];
            }
            ...
            pdf_setfont($pdf, $font, 12);
            pdf_set_text_pos($pdf, 175, 775 - $pos3);
            pdf_continue_text($pdf, $row['memid']);
            pdf_setfont($pdf, $font, 8);
            pdf_set_text_pos($pdf, 62, 760 - $pos3);
            pdf_continue_text($pdf, 'Please Use Account Number For Payment Reference.');
            //pdf_continue_text($pdf, " ");
             pdf_setfont($pdf, $font, 12);
            ...
            if (!isset($_REQUEST['invoice'])) {
                ...
                if ($rollover) {
                    $Newdet = explode("|", wordwrap("Real Estate Rollover Fee", 70, "|"));
                    foreach ($Newdet as $Line) {
                        pdf_set_text_pos($pdf, get_left_pos($Line, $pdf, "225", 10, $font), $pos);
                        pdf_continue_text($pdf, $Line);
                        $pos = $pos - 10;
                    }
                } else {
            ...
            if (!isset($_REQUEST['invoice']) && !$rollover) {
                pdf_setcolor($pdf, "fill", "rgb", 0, 0, 0, 0);
                pdf_setfont($pdf, $font, 7);
            ...
            if ($row['tax'] != 0) {
                pdf_set_text_pos($pdf, get_right_pos(decimal_format($gst, 1, '', '', $row['CID']), $pdf, "520", 10, $font), 480);
            ...
            pdf_set_text_pos($pdf, get_right_pos($nett, $pdf, "520", 10, $font), 505);
            pdf_continue_text($pdf, $nett);
            pdf_set_text_pos($pdf, get_right_pos(decimal_format($row['currentfees'], 1, '', '', $row['CID']), $pdf, "520", 10, $font), 455);
            ...
            if ($row['CID'] != 15) {
                pdf_set_text_pos($pdf, get_right_pos($row['overduefees'], $pdf, "520", 10, $font), 425);
                pdf_continue_text($pdf, $row['overduefees']);
                pdf_set_text_pos($pdf, get_right_pos($currentpaid, $pdf, "520", 10, $font), 396.5);
                pdf_continue_text($pdf, $currentpaid);
                pdf_set_text_pos($pdf, get_right_pos($total, $pdf, "520", 10, $font), 368);
                pdf_continue_text($pdf, $total);
            }

            if (!$rollover && $row['paymenttype'] && $row['accountno']) {
                pdf_set_text_pos($pdf, get_right_pos("Direct Debit in place - NO PAYMENT NEEDED", $pdf, "520", 10, $font), 345);
                pdf_continue_text($pdf, "Direct Debit in place - NO PAYMENT NEEDED");
                pdf_setfont($pdf, $font, 8);
                pdf_continue_text($pdf, "Please use account number as reference for any direct credit payments.");
            }
            ...
            if ($row['CID'] == 15) {
                ...
                pdf_set_text_pos($pdf, get_left_pos("Inscrito Registro Mercantil Tomo 2964 folio 95 Hoja A-94034 El 23.09.2005", $pdf, "302.5", 6, $font), 220);
            ...
            if ($row['CID'] == 1 && $stationery) {
                ...
                //pdf_rect($pdf, 65, 339, 200, 130);
                pdf_rect($pdf, 65, 209, 200, 140);
                pdf_closepath_stroke($pdf);

                pdf_moveto($pdf, 65, 332 - $offset);
                pdf_lineto($pdf, 265, 332 - $offset);
                pdf_stroke($pdf);

                pdf_setfont($pdf, $font, 10);
                pdf_set_text_pos($pdf, get_left_pos("PAYMENT METHODS", $pdf, "165", 8, $font), 345);
                pdf_continue_text($pdf, "PAYMENT METHODS");

                pdf_setfont($pdf, $font, 8);
                pdf_set_text_pos($pdf, 70, 290);
                pdf_continue_text($pdf, "Credit Card");

                //pdf_set_text_pos($pdf, 70, 447);
                pdf_set_text_pos($pdf, 70, 327);
                pdf_continue_text($pdf, "Cheque");

                //pdf_set_text_pos($pdf, 70, 432);
                /* pdf_set_text_pos($pdf, 70, 308);
                  pdf_continue_text($pdf, "Cash"); */

                //pdf_set_text_pos($pdf, 70, 412);
                //pdf_continue_text($pdf, "Direct Deposit");
                //pdf_set_text_pos($pdf, 130, 290);
                //pdf_continue_text($pdf, "- Call 1800 675 092");
                //pdf_continue_text($pdf, "- online at www.etxint.com");
                //pdf_setfont($pdf, $font, 7);
                //pdf_continue_text($pdf, " (log into secure member section)");

                pdf_setfont($pdf, $font, 8);
                pdf_set_text_pos($pdf, 130, 327);
                pdf_continue_text($pdf, "- mail to PO Box above");

                /* pdf_set_text_pos($pdf, 130, 308);
                  pdf_continue_text($pdf, "- in person"); */
            ...
            if ($row['CID'] == 1) {
                if (!$stationery) {
                    ...
                    $pdfimage = pdf_load_image($pdf, "jpeg", "/home/etxint/public_html/home/images/BPAY.jpg", '');
                    pdf_fit_image($pdf, $pdfimage, 300, 188, "scale 1.23");
                    ...
                    pdf_continue_text($pdf, " ABN 88 611 249 640");
                } else {
...
//		    $pdfimage = pdf_load_image($pdf, "jpeg", "/home/etxint/public_html/home/images/BPAY.jpg",'');
//			pdf_fit_image($pdf, $pdfimage, 80, 218, "scale 0.3");

                    pdf_setfont($pdf, $fontbold, 10);
                    pdf_set_text_pos($pdf, 75, 253);
                    pdf_continue_text($pdf, "Direct");

                    pdf_setfont($pdf, $fontbold, 10);
                    pdf_set_text_pos($pdf, 75, 238);
                    pdf_continue_text($pdf, "Debit");

                    /*
                     *
                     * Empire Trade Exchange Pty Ltd  ( A B N  88611249640)
                    ...
                    pdf_rect($pdf, 120, 212, 142, 47);
                    pdf_closepath_stroke($pdf);

                    pdf_setfont($pdf, $font, 9);
                    pdf_set_text_pos($pdf, $textLeft, 257);
                    pdf_continue_text($pdf, "Access Commercial Exchange");  //Trade Exchange Pty Ltd

                    pdf_setfont($pdf, $fontbold, 9);
                    pdf_set_text_pos($pdf, $textLeft, 243);
                    pdf_continue_text($pdf, "Westpac BSB:");

                    pdf_setfont($pdf, $fontbold, 9);
                    pdf_set_text_pos($pdf, $textLeft, 230);
                    pdf_continue_text($pdf, "Account:");

                    pdf_setfont($pdf, $font, 9);
                    pdf_set_text_pos($pdf, $textLeft + 75, 243); // it was 55 before on 75 place
                    pdf_continue_text($pdf, "033 077");

                    pdf_setfont($pdf, $font, 9);
                    pdf_set_text_pos($pdf, $textLeft + 75, 230); // it was 55 before on 75 place
                    pdf_continue_text($pdf, "380428");
...
//			pdf_set_text_pos($pdf, 128, 253);
//			pdf_continue_text($pdf, "Biller Code: 374215");
//
//			$ref = "Ref: ".bpay_code($row[memid]);
//			//pdf_set_text_pos($pdf, 348, 328);
//			pdf_set_text_pos($pdf, 128, 238);
//			pdf_continue_text($pdf, $ref);
                }
            }
