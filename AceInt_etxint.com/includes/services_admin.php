...
<form method="POST" action="body.php?page=services_admin&ID=<?= $_GET[ID] ?>&tab=<?= $_GET[tab] ?>" name="frmletters">
...
function letters($letterno, $pend = false) {
 ...
 if($_REQUEST['payment']) {
   ...
   $query = dbRead("select * from registered_accounts where FieldID = ".$_REQUEST['FieldID']."","ebanc_services");
...
<table width="100%" border="0" cellpadding="3" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111">
 ...
 <?
   ...
   if($_REQUEST['process']) {
    ...
    $query = dbRead("select registered_accounts.*, plans.Plan_Fee as Plan_Fee from registered_accounts, plans where (registered_accounts.Plan_ID = plans.FieldID) and registered_accounts.FieldID = ". $_REQUEST['FieldID'] ."","ebanc_services");
    $row = mysql_fetch_array($query);
    ...
    if($_REQUEST['cash']) {
     dbWrite("update registered_accounts set cashToPay = ". $cc .", creditReceiptNo = '". $_REQUEST['method'] ."' where FieldID = ". $row['FieldID'] ."","ebanc_services");  
   	 dbWrite("insert into transactions (regAccID, sell, type_id, details, receipt, dis_date, dollarfees) values ('" . $_REQUEST['FieldID'] . "','" . $aa . "','1','Cash Payment - " . $_REQUEST['FieldID'] . "','" . $_REQUEST['method'] . "','" . date("Y-m-d") . "','" . $row['Plan_Fee'] . "')", "ebanc_services");
    }
    
    if($_REQUEST['trade']) {
     dbWrite("update registered_accounts set tradeToPay = ". $tt .", tradeReceiptNo = '". $_REQUEST['auth'] ."' where FieldID = ". $row['FieldID'] ."","ebanc_services");  
 	 dbWrite("insert into transactions (regAccID, sell, type_id, details, receipt, dis_date) values ('" . $_REQUEST['FieldID'] . "','" . $_REQUEST['trade'] . "','2','Trade Payment - " . $_REQUEST['FieldID'] . "','" . $_REQUEST['auth'] . "','" . date("Y-m-d") . "')", "ebanc_services");
    } 
            
	if(($cc - $tt) == 0) {
      dbWrite("update registered_accounts set Status_ID = 1, Date_Paid = '". date("Y-m-d")."' where FieldID = ". $row['FieldID'] ."","ebanc_services");    
	} 
  }
 ...
 if($_REQUEST[B1]) {
  ...
  // send the message.
   //$mail->send($_SESSION['User']['Name'], $_SESSION['User']['EmailAddress'], 'E Banc Accounts - Head Office', 'accounts@ebanctrade.com', 'Letters - 1');
    ...
    foreach($MemberArray as $key => $value) {
     ...
     dbWrite("update registered_accounts set Statsus = '1' where memid = '$value'");
    ...
    foreach($MemberArray as $key => $value) {
     ...
     dbWrite("update registered_accounts set Status_ID = '2' where FieldID = '$value'","ebanc_services");
     if($pend && $CardArray[$value] > 0) {
      dbWrite("update registered_accounts set Card_No = '".$CardArray[$value]."' where FieldID = '$value'","ebanc_services");
     }
  ...
  // send the message.
   //$mail->send($_SESSION['User']['Name'], $_SESSION['User']['EmailAddress'], 'E Banc Accounts - Head Office', 'accounts@ebanctrade.com', 'Letters - '.$letterno2);
    ...
    foreach($MemberArray as $key => $value) {
     dbWrite("update registered_accounts set Status = '3' where memid = '$value'");
