...
if($_REQUEST[view] && $gg) {
 ...
 $query = dbRead("select * from invoice, members, country, countrydata where invoice.memid=members.memid and members.CID = countrydata.CID and members.CID=country.countryID and invoice.memid = '$_REQUEST[memid]' and invoice.date like '$_REQUEST[currentyear]-$_REQUEST[currentmonth]-%'");

 if (@mysql_num_rows($query) > 0) {

   dbWrite("insert into notes (memid,date,userid,type,note) values ('".$_REQUEST[memid]."','".date("Y-m-d")."','".$_SESSION['User']['FieldID']."','1','Statement Printed')");
...
if($_REQUEST[send] && $jj)  {
 ...
 $query2 = dbRead("select members.memid as memid, members.companyname as companyname, tbl_members_email.email as emailaddress, members.contactname as contactname, members.CID as CID from invoice, members, tbl_members_email where invoice.memid=members.memid and (members.memid = tbl_members_email.acc_no) and invoice.date like '$_REQUEST[currentyear]-$_REQUEST[currentmonth]-%' and tbl_members_email.type = 2 and members.memid = '$_REQUEST[memid]'");
 ...
 while($row2 = mysql_fetch_assoc($query2)) {
  ...
  if ($row2['emailaddress']) {
    ...
    // send the message.
    $mail->send($row2[contactname], $row2[emailaddress], 'E Banc Accounts', 'accounts@ebanctrade.com', 'Statement - '.$row2[companyname],'Bcc: dave@ebanctrade.com');
    ...
    echo "Statement has been email to $row2[emailaddress]";
   } else {
...
<form method="post" action="../includes/statement.php" name="TSel">
...
<table border="0" cellpadding="3" cellspacing="0" width="100%">
	<tr>
		<td colspan="2" align="center" class="Heading2">aaaaaaaaaaa<?= get_word("209") ?></td>
	</tr>
	</tr>
		<td class="Heading2" width="150" align="right"><b><?= get_word("50") ?>:</b></td>
	...
	<tr>
		...
		<td width="450" bgcolor="#FFFFFF">
        <input type="submit" value="<?= get_word("221") ?>" name="view" style="size: 8pt">
        <input type="submit" value="<?= get_word("220") ?>" name="send" style="size: 8pt"></td>
	</tr>
</table>
</td>
</tr>
