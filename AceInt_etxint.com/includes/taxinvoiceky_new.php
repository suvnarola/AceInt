...
function taxinvoice_data_pdf($query_data, $stationery = true, $individual = false, $send = false, $rollover = false) {
    ...
    foreach ($query_data as $key_number => $row) {
        if ($row['CID'] == 151) {
...
//lines and boxes
            ...
            if ($stationery || ($individual && $send)) {
                ...
                pdf_set_text_pos($pdf, get_right_pos("$row[acno]:", $pdf, "170", 12, $font), 773 - $pos3);
                pdf_continue_text($pdf, "$row[acno]:");
