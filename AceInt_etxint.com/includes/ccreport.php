...
if($_REQUEST['next'] == 2) {
 ...
 second_form($_REQUEST['day1'],$_REQUEST['month1'],$_REQUEST['year1']);
...
function second_form($day,$month,$year) {
...
<table width="100%" border="0" cellpadding="3" cellspacing="0">
	...
	<tr>
		<td class="Heading2"><b>ID:</b></font></td>
		...
		<td class="Heading2"><b><?= get_page_data("2") ?>:</b></font></td>
	...
	//$query = dbRead("select credit_transactions.memid as memid, card_name as cname, credit_transactions.date as date, credit_transactions.success as success, credit_transactions.amount as amount, credit_transactions.response_code as response_code, credit_transactions.type as type, members.companyname as companyname from credit_transactions, members where (credit_transactions.memid = members.memid) and date like '$year-$month-$day' and success not like 'In_Progress' and type != 9 and type != 12 order by type");
	$query = dbRead("select credit_transactions.memid as memid, card_name as cname, credit_transactions.date as date, credit_transactions.success as success, credit_transactions.amount as amount, credit_transactions.response_code as response_code, credit_transactions.type as type, members.companyname as companyname, credit_transactions.FieldID as FieldID from credit_transactions, members where (credit_transactions.memid = members.memid) and date like '$year-$month-$day' and success not like 'In_Progress' order by type, FieldID");
	...
	<tr bgcolor="<?= $bgcolor ?>">
		<td><?= $row['FieldID'] ?></td>
		<td><?= $new_date ?></td>
		<td><?= get_all_added_characters($DisplayName) ?></td>
		<td><?= get_all_added_characters($row['cname']) ?></a></td>
		<td><?= get_all_added_characters($row['success']) ?></a></td>
	...
	if(@mysql_num_rows($query) > 0) {
	 ...
	 ?>
 	 <tr bgcolor="#FFFFFF">
		<td colspan="7" align="right"><b><?= $Type ?>:</b></td>
...
<form name="ccreport" method="POST" action="body.php?page=ccreport">
...
<table width="400" border="0" cellspacing="0" cellpadding="3" align="center">
  <tr>
    <td colspan="2" class="Heading2" align="center"><b><?= get_page_data("1") ?>.</b></td>
  </tr>
  <tr>
    <td width="100" align="right" class="Heading2"><b><?= get_word("41") ?>: </b></td>
    <td bgcolor="#FFFFFF">
      <select name="day1">
